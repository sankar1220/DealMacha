...
public class UsersService implements IUsersService {
	...
	/*
	 * @Autowired private IUserImagesService userImagesService;
	 */
	@Autowired
	...
	/*  *//**
			 * {@inheritDoc}
			 *
			 * @see com.chitfund.service.IUsersService#addImages(com.chitfund.domain.Users,
			 *      java.util.List)
			 */
	/*
	 * @Override public Users addImages(final Users users, final
	 * List<UserImages> userImages) { Validate.notNull(users,
	 * "user must not be null"); Set<UserImages> addImages = new
	 * HashSet<UserImages>(userImages);
	 * 
	 * for (UserImages uImages : userImages) { UserImages userImages1 = new
	 * UserImages(); String s = uImages.getImageName();
	 * 
	 * if (uImages.getId() != null && uImages.getId() != "") { UserImages
	 * userImages2 = userImagesRepository.findOne(uImages.getId()); s =
	 * s.replaceAll("\\\\", "/"); userImages2.setImageName(s);
	 * userImages2.setImageType(uImages.getImageType());
	 * userImages2.setImageLocation(uImages.getImageLocation()); userImages1 =
	 * userImagesRepository.save(userImages2); } else { s = s.replaceAll(
	 * "\\\\", "/"); userImages1.setImageName(spImages.getImageLocation());
	 * userImages1.setImageType(uImages.getImageType());
	 * userImages1.setImageLocation(uImages.getImageLocation()); userImages1 =
	 * userImagesRepository.save(userImages1); }
	 * 
	 * } users.setUserImageses(addImages); return users; }
	 */
	/**
	 * {@inheritDoc}
	 *
	 * @see com.chitfund.service.IUsersService#addRoles(com.chitfund.domain.Users,
	 *      java.util.List)
	 */
	/*
	 * @Override public Users addRoles(final Users users, final List<UserRole>
	 * userRoles) { Validate.notNull(users, "user must not be null"); Users
	 * newUser = new Users(); Set<UserRole> addRoles = new
	 * HashSet<UserRole>(userRoles); newUser.setId(users.getId());
	 * usersRepository.save(newUser); return newUser; }
	 */

	/**
	 * {@inheritDoc}
	 *
	 * @see com.dealmacha.service.IUsersService#addUsersAddress(com.dealmacha.domain.Users,
	 *      java.util.Set)
	 */
	@Override
	 ...
	 * {@inheritDoc}
	 *
	 * @see com.chitfund.service.IUsersService#addUserImages(com.chitfund.domain.Users,
	 *      java.util.List)
	 */
	/*
	 * @Override public Users addUserImages(final Users users, final
	 * List<UserImages> userImag) { Validate.notNull(users,
	 * "users must not be null"); Set<UserImages> addImages = new
	 * HashSet<UserImages>(userImag); for (UserImages uImages : userImag) {
	 * UserImages userImages = new UserImages(); String s =
	 * uImages.getImageName(); s = s.replaceAll("\\\\", "/"); if
	 * (uImages.getId() != null) { userImages =
	 * userImagesService.getUserImages(uImages.getId());
	 * userImages.setId(userImages.getId()); userImages.setImageName(s);
	 * userImages.setImageType(uImages.getImageType());
	 * userImages.setImageLocation(uImages.getImageLocation());
	 * userImages.setUsers(userImages.getUsers()); userImages =
	 * userImagesService.updateUserImages(userImages); } else {
	 * userImages.setImageName(s);
	 * userImages.setImageType(uImages.getImageType());
	 * userImages.setImageLocation(uImages.getImageLocation());
	 * userImages.setUsers(users); addImages.add(userImages); userImages =
	...
	public Users changesignup(final String emailId, final String newPassword,
		...
		if (ud1 != null) {
			...
			if (!newPassword.isEmpty()) {
				...
				/*
				 * Mail mail = new Mail(); mail.setMailFrom(mailFrom);
				 * mail.setMailTo("to@gmail.com"emailId); mail.setMailSubject(
				 * "Password Changed Successfully"); sendMail(mail,emailId);
				 */
	 ...
	 * {@inheritDoc}
	 *
	 * @see com.dealmacha.service.IUsersService#findByChangePassword(java.lang.String,
	 *      java.lang.String, java.lang.String, java.lang.String)
	 */
	@Override
	public Users getByChangePassword(final String userId, final String confirmPassword, final String newPassword,
	 ...
	 * {@inheritDoc}
	 *
	 * @see com.chitfund.service.IUsersService#findByEmailIdAndForgotPassword(java.lang.String,
	 *      java.lang.String)
	 */
	@Override
	public Users findByEmailIdAndForgotPassword(final String emailId, final String forgotPasswordStatus) {
		...
		/*
		 * Mail mail = new Mail(); mail.setMailFrom(mailFrom);
		 * mail.setMailTo("to@gmail.com"user.getEmailId()); mail.setMailSubject(
		 * "Subject - Reset Your Password"); mail.setTemplateName("forgot.vm");
		 * 
		 * String html = "<p>Hi!</p><a href=\"google.com\">Link text</a>";
		 * 
		 * String firmName = null; for (FirmBranch fb : user.getFirmBranchs()) {
		 * firmName = fb.getFirmDetails().getFirmName(); }
		 * mail.setFirmName(firmName); sendMail(mail, user); }
		 */
		else {
	 ...
	 * {@inheritDoc}
	 *
	 * @see com.chitfund.service.IUsersService#findByEmailIdAndPassword(java.lang.String,
	 *      java.lang.String)
	 */
	@Override
	 ...
	 * {@inheritDoc}
	 *
	 * @see com.chitfund.service.IUsersService#findByMobileNoAndPassword(java.lang.String,
	 *      java.lang.String)
	 */
	@Override
	 ...
	 * {@inheritDoc}
	 *
	 * @see com.chitfund.service.IUsersService#getByForgotPasswordEmail(java.lang.String,
	 *      java.lang.String)
	 */
	@Override
	public List<Users> getByForgotPasswordEmail(final String emailId, final String forgotPasswordStatus) {
		 ...
		 * mailMessage.setTo(emailId); String replyTo =
		 * "contact@electrikals.com"; String from = "contact@electrikals.com";
		 * String subject = "Test Mail Sender"; String SALT = "ZYX123abc";
		 * String saltedPassword = user.getPassword().concat(SALT);//+"SALT";
		 * String hashedPassword = SimpleMD5Example(saltedPassword);
		 * 
		 * String text = "Hi " + user.getFirstName().concat(user.getLastName())
		 * + "your email Id is " + user.getEmailId() + " and  password is" +
		 * hashedPassword; mailMessage.setReplyTo(replyTo);
		 * mailMessage.setFrom(from); mailMessage.setSubject(subject);
		 * mailMessage.setText(text); Send s = new Send(); s.send(emailId,
		 * mailMessage.getReplyTo(), mailMessage.getFrom(),
		...
		if (user != null) {
			 ...
			 */
			String html = "<p>Hi!</p><a href=\"" + url + "/getForgotPassword.html#" + user.getId() + "?em="
					+ user.getEmailId() + "\">Link text</a>";
	...
	/*
	 * public void sendMail(final Mail mail, final Users ud) { SimpleMailMessage
	...
	/*
	 * String firmName = null; for (FirmBranch fb : ud.getFirmBranchs()) {
	 * firmName = fb.getFirmDetails().getFirmName(); } String html =
	 * "<p>Hi!</p><a href=\"" + url + "/getForgotPassword.html#" + ud.getId() +
	 * "?em=" + ud.getEmailId() + "\">Link text</a>";
	 * velocityContext.put("html", html);
	 */
	// System.out.println("firm details are "+firmName);
	// velocityContext.put("firmName", firmName());
	/*
	 * velocityContext.put("firmName", firmName); StringWriter stringWriter =
	 * new StringWriter(); template.merge(velocityContext, stringWriter);
	 * message.setText(stringWriter.toString());
	 * 
	 * MimeMessagePreparator preparator = new MimeMessagePreparator() {
	 * 
	 * @Override public void prepare(final MimeMessage mimeMessage) throws
	 * Exception { MimeMessageHelper message = new
	 * MimeMessageHelper(mimeMessage); message.setTo(mail.getMailTo());
	 * message.setSubject(mail.getMailSubject());
	 * message.setFrom(mail.getMailFrom());
	 * 
	 * Map map = new HashMap(); map.put("firstName", ud.getFirstName());
	 * map.put("lastName", ud.getLastName()); map.put("userId", ud.getId());
	 * String html = url + "/getForgotPassword.html#" + ud.getId() + "?em=" +
	 * ud.getEmailId(); map.put("html", html);
	 * 
	 * String text = VelocityEngineUtils.mergeTemplateIntoString(velocityEngine,
	 * "templates/forgot.vm", "UTF-8", map);
	 * 
	...
	/*
	 * @Override public Users addAccount(Users users, Set<Account> accounts) {
	 * Validate.notNull(users, "users must not be null"); Set<Account>
	 * addAccounts = new HashSet<Account>(accounts); for (Account acc :
	 * accounts) { Account acct = new Account();
	 * acct.setCashbackAmount(acc.getCashbackAmount()); acct.setUsers(users);
	 * addAccounts.add(acct); } users.setAccounts(addAccounts); return users; }
	 */
	...
	@Override
	public Users getUsersByEmailId(String emailId) {
		Users users = new Users();
